...
from stargaze_ml.gold.l2_dominance_override import (
    DominanceOverrideConfig, simulate_live_dominance_swaps,
)
...
def main() -> None:
    ...
    val_rows = _trade_rows(model, teacher, data, normalizer, _event_indices(data, data.train_end, data.validation_end, good_only=False), threshold, device, market, config)
    test_rows = _trade_rows(model, teacher, data, normalizer, _event_indices(data, data.validation_end, len(data.x), good_only=False), threshold, device, market, config)
    ...
    grid = [{"config": None, "swaps": 0, "selection_score": robust_validation_score(baseline), **baseline}]
        ...
        for alignment in (0.6, 0.8, 1.0):
            for slope_consensus in (0.6, 0.8, 1.0):
                for minimum in (0.0, 1.0, 3.0, 5.0):
                    for confirmation in (2, 3, 5, 10):
                        swap_config = DominanceOverrideConfig(
                            horizons=horizons, alignment_consensus=alignment,
                            slope_consensus=slope_consensus,
                            min_projected_slope_ticks=minimum,
                            confirmation_seconds=confirmation, max_swaps=1,
                        )
                        changed, swaps = simulate_live_dominance_swaps(val_selected, data, market, swap_config)
                        ...
                        grid.append({
                            ...
                            "swaps": swaps, "selection_score": robust_validation_score(metrics), **metrics,
                        })
    ...
    if selected["config"] is None:
        ...
        fixed_rows, fixed_swaps = simulate_live_dominance_swaps(
            test_selected, data, market, DominanceOverrideConfig(**selected["config"])
        )
    report = {
        "validation_baseline": baseline, "selected_on_validation": selected,
        "fixed_test_swaps": fixed_swaps, "fixed_test": summarize_selected(fixed_rows),
